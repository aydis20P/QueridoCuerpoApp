...
def principal(request):
     ...
     if m:
          code = m.group(1)
          request.session['id_strava'] = code
          print(code)
     if not request.session['id_strava'] == "":
...
def resumen_usuario(request):
     ...
     try:
          ...
          response = requests.post(
                              ...
                              data = {
                                   'client_id': 63232,
                                   'client_secret': '042d93e39aa4abe4ea68fe237c6ac15bdca3261a',
                                   'code': '7e7515da94683e96d68b0b6395d7d4a86723bceb',
                                   'grant_type': 'authorization_code'
                                   }
                         )
          #Save json response as a variable
     ...
     #OPTION: get 1 page with last 5 activities from Strava and filtering
     page = 1
     # get page of activities from Strava
     r = requests.get(url + '?access_token=' + access_token + '&per_page=5' + '&page=' + str(page)) #***pages***
     r = r.json()
